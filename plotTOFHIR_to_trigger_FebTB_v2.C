...
// to run: ./plotTOFHIR_to_triggerFebTBv2 24702 24712
...
//  25,24,26,28,29,30,31,32,27,11,2,13,8,15,6,4}; // VERTICAL
  ...
  std::map<float, std::map<float, std::map<int, TH1F * > > > hEff_vs_X_noMIP;
      ...
      for (int iStep2 = 0; iStep2< NSTEP2; iStep2++)
	{
	  for (int iCh = 0; iCh < NCH; iCh++)
	    {   
	      ...
	      hEff_vs_X_noMIP[step1_vct.at(iStep1)][step2_vct.at(iStep2)][iCh]  = new TH1F (Form("hEff_vs_Xpos_noMIP_%s", this_caption.c_str()), Form("Efficiency vs. X pos %s", this_title.c_str()), NBINPOS, minXpos, maxXpos);
  ...
  for (Int_t iEvt= 0; iEvt < NEVENTS; iEvt++) 
      ...
      for (int iCh = 0; iCh<NCH; iCh++) // channel loop in the event loop
	{
	  ...
	  hEff_vs_X_noMIP[step1][step2][iCh]->Fill(xIntercept);
	  ...
	  // x and y intercepts vs energy for each channel
  ...
  gPad->SetLogy();
  cBeamPosX->SaveAs(Form("Beam X Position.pdf"));
  ...
  cArrayTots->SaveAs(Form("ToT Array.pdf"));
  ...
  cTotsZoom->SaveAs(Form("ToT Top and Bottom Array Center Bar.pdf"));      
  ...
  cArrayEnergies->SaveAs(Form("Energies with Landau Fit.pdf")); 
  ...
  gStyle->SetOptStat(0);
  cEnergiesZoom->SaveAs(Form("Energy Top and Bottom Array Center Bar.pdf"));

  TCanvas * cEnergiesZoomLog = new TCanvas ("cEnergiesZoomLog", "cEnergiesZoomLog", 500, 800);
  cEnergiesZoomLog->Divide(1, 2);
  cEnergiesZoomLog->cd(1);
  hEnergy[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChTop[selBarNumber]]->Draw();
  hEnergy_cut[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChTop[selBarNumber]]->Draw("same");
  gPad->SetLogy();
  cEnergiesZoomLog->cd(2);
  hEnergy[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChBot[selBarNumber]]->Draw();
  hEnergy_cut[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChBot[selBarNumber]]->Draw("same");
  cEnergiesZoomLog->SaveAs(Form("Energy Top and Bottom Array Center Bar.pdf"));
  gPad->SetLogy();
  gStyle->SetOptStat(0);
  cEnergiesZoomLog->SaveAs(Form("Energy Top and Bottom Array Center Bar, Log.pdf"));  
  ...
  for (int chId = 0; chId<NBARS*2; chId++) 
    {
      IC[chId] = MIP_peak[chId] / avgMIP; // intercalibration coefficient - ratio of MIP peak to average MIP peak value
      if (chId<NBARS) 
        {
  ...
  gMIP_top->GetXaxis()->SetTitle("Bar Number");
  ...
  leg->Draw();
  gPad->SetGridy();
  cMIP_peak->SaveAs(Form("MIP Peak Values.pdf"));
  
  TCanvas * cIC = new TCanvas ("cIC", "cIC", 600, 500);
  cIC->cd();
  gIC_top->Draw("APE");
  gIC_top->GetYaxis()->SetRangeUser(0.7, 1.3);
  gIC_top->GetYaxis()->SetTitle("IC = MIP peak / avg MIP peak");
  gIC_top->GetXaxis()->SetTitle("Bar Number");
  ...
  cArrayEnergy_vsXY->SaveAs(Form("2D Energy vs Position.pdf"));
  ...
  cEnergy_vsXYZoom->SaveAs(Form("2D Energy vs Position, Center Bar.pdf"));
  ...
  cArrayEnergy_vsPos->SaveAs(Form("Energy vs. Position.pdf"));
  ...
  cEnergy_vsPosZoom->SaveAs(Form("Energy vs. Position, Center Bar.pdf"));
  ...
  cArrayEfficiency_vsXY->SaveAs(Form("2D Efficiency vs XY.pdf"));
  ...
  cEfficiency_vsXYZoom->SaveAs(Form("2D Efficiency vs XY, Center Bar.pdf"));
  ...
  cArrayEfficiency_vsPos->SaveAs(Form("Efficiency vs. X Position.pdf"));
  ...
  for (int chId = 0; chId<NBARS; chId++)
    {
      ...
      hEff_vs_X_noMIP[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChTop[chId]]->Rebin(2);
  ...
  for (int chId = 0; chId<NBARS; chId++)
    {
      ...
      hEff_vs_X_noMIP[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChBot[chId]]->Rebin(2);
	  ...
	  //	  if (hEff_vs_X_noMIP[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChBot[chId]]->GetBinContent(iBin+1)>0) hEff_vs_X[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChBot[chId]]->SetBinContent(iBin+1, hEff_vs_X[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChBot[chId]]->GetBinContent(iBin+1)/hEff_vs_X_noMIP[step1_vct.at(selStep1)][step2_vct.at(selStep2)][myChBot[chId]]->GetBinContent(iBin+1) );
        }
  ...
  std::cout << "average efficiency from good fits = " << avgEff/numEff << std::endl;
  ...
  BarSpacingTop->SetFillColor(kRed+1);
  ...
  gPad->SetGridx();
  cBarSpacing->SaveAs(Form("Bar Spacing Hist.pdf"));
  ...
  for (Int_t iEvt= 0; iEvt < NEVENTS; iEvt++) 
    {
	  ...
	  if (   energy[myChList[iBar]] > MIP_peak[iBar]*0.8 && energy[myChList[iBar]] < MIP_peak[iBar]*5 && energy[myChList[iBar+16]] > MIP_peak[iBar+16]*0.8 && energy[myChList[iBar+16]] < MIP_peak[iBar+16]*5 )
	    {
  ...
  TGraphErrors * gXT_thickness_top = new TGraphErrors(); 
  TGraphErrors * gXT_thickness_bot = new TGraphErrors();
  ...
  for (int chId = 0; chId<NBARS*2; chId++) 
    {
	  ...
	  gXT_top->SetPointError(chId, 0, XT_err[chId]);
          gXT_thickness_top->SetPoint(chId, (1.005-0.963)/15*(chId) + 0.963, XT[chId]); // resin measurements for the milano (flex) array
          gXT_thickness_top->SetPointError(chId, 0, XT_err[chId]);
	  if(XT_ln[chId]>0 && XT_err_ln[chId] < 0.2)// && chId > 2)
            {
	      gXT_top_ln->SetPoint(chId, chId, XT_ln[chId]);
	      gXT_top_ln->SetPointError(chId, 0, XT_err_ln[chId]);
            }
	  if(XT_rn[chId]>0 && XT_err_rn[chId] < 0.2)// && chId > 2)
            {
  ...
  leg->Draw();
  gPad->SetGridy();
  cXT_abs->SaveAs("XT Fraction of Light in bar.pdf");
  
  TCanvas * cXT_thickness_abs = new TCanvas ("cXT_thickness_abs", "cXT_thickness_abs", 600, 500);
  cXT_thickness_abs->cd();
  gXT_thickness_bot->Draw("APE");
  gXT_thickness_bot->GetYaxis()->SetRangeUser(0.5, 1.);
  gXT_thickness_bot->GetYaxis()->SetTitle("Fraction of light within bar (XT)");
  gXT_thickness_bot->GetXaxis()->SetTitle("Estimated Resin Thickness");
  gXT_thickness_bot->SetMarkerStyle(21);
  gXT_thickness_bot->SetMarkerColor(kRed+1);
  gXT_thickness_bot->SetLineColor(kRed+1);

  gXT_thickness_top->SetMarkerStyle(20);
  gXT_thickness_top->SetMarkerColor(kBlue+1);
  gXT_thickness_top->SetLineColor(kBlue+1);
  gXT_thickness_top->Draw("same PE");

  leg = new TLegend(0.15,0.75,0.5,0.88,NULL,"brNDC");
  leg->AddEntry(gXT_thickness_top, "top SiPMs", "lp" );
  leg->AddEntry(gXT_thickness_bot, "bottom SiPMs", "lp" );
  leg->Draw();
  gPad->SetGridy();
  cXT_thickness_abs->SaveAs("XT Fraction of Light in bar vs resin thickness.pdf");
  ...
  leg->Draw();
  gPad->SetGridy();
  cXT_abs_nb->SaveAs(Form("XT Fraction of Light in Neighboring Bar.pdf"));
  ...
  leg->Draw();
  gPad->SetGridy();
  cXT_norm->SaveAs(Form("Fraction of Light over XT.pdf"));
  ...
  gPad->SetGridx();
  cXT_norm_RMS->SaveAs(Form("XT normalized.pdf"));
  ...
  cArrayCTR_UD->SaveAs(Form("Time in each Bar.pdf"));
  ...
  gPad->SetGridy();
  cCTR_UD_abs->SaveAs("cCTR_UD_abs.pdf");
  ...
  gPad->SetGridy();
  cCTR_UD_norm->SaveAs("cCTR_UD_norm.pdf");
